    ...
    >
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <div className="relative ml-auto">
          <label htmlFor="search-input" className="sr-only">{localize('搜索活动标题或地点')}</label>
          ...
          <input
            ...
            placeholder={localize('搜索标题或地点...')}
            className="input w-48 py-1.5 pl-8 pr-3 text-sm"
          />
        </div>
      </div>
      <p className="text-xs text-brand-fg/45">
        {localize('当前筛选')}: {localize(statusLabel[statusFilter] || '全部')} · {localize('显示')} {filteredEvents.length} {localize('条')}
      </p>
      ...
      {loading ? (
        ...
        <EmptyState
          icon={CalendarDays}
          title={localize('暂无活动记录')}
          description={localize('点击右上角“创建活动”来新增第一条记录')}
        />
      ) : filteredEvents.length === 0 ? (
        <EmptyState
          icon={CalendarDays}
          title={localize('未找到匹配的活动')}
          description={localize('可以尝试调整您的搜索关键词')}
        />
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-brand/10 text-main/60">
              <tr>
                ...
                <th className="px-4 py-3 font-medium hidden sm:table-cell">{localize('地点')}</th>
                <th className="px-4 py-3 font-medium hidden md:table-cell">{localize('活动时间')}</th>
                <th className="px-4 py-3 font-medium">{localize('报名')}</th>
              ...
              {filteredEvents.map((item) => (
                <tr key={item.id} className="text-main/70 transition hover:bg-brand/5">
                  ...
                  <td className="px-4 py-3 hidden md:table-cell">
                    {new Date(item.eventDate).toLocaleString(locale === 'en' ? 'en-US' : 'zh-CN', { timeZone: 'Asia/Shanghai' })}
                  </td>
                  <td className="px-4 py-3">
                    <span className="inline-flex items-center gap-1 text-xs text-main/60">
                      <Users size={14} />
        ...
        >
          <div className="w-full max-w-md rounded-modal border border-line bg-surface p-6 shadow-lg backdrop-blur-md mb-4 sm:mb-0 animate-slide-in sm:animate-fade-in">
            ...
            <p className="mt-3 text-sm leading-6 text-main/70">
              {locale === 'en' ? `Delete event “${confirmDelete.title}”? This action cannot be undone.` : `确定删除活动《${confirmDelete.title}》吗？此操作不可撤销。`}
            </p>
