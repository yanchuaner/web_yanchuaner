...
export default function AdminEventsNewPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(localize('活动创建成功'));
      router.push('/admin/events');
    } catch (err: any) {
      setError(err.message);
      toast.error(err.message || localize('创建失败'));
      setSubmitting(false);
    }
  };
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-brand/10 bg-surface/50 p-6 backdrop-blur-sm">
        ...
        <div>
          ...
          {form.coverImage && (
            <div className="relative mt-2 aspect-video w-full max-w-64 overflow-hidden rounded-lg border border-brand/10">
              <Image
                src={form.coverImage}
                alt={localize('封面预览')}
                fill
