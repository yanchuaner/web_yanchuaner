  ...
  return (
    <main className="flex min-h-screen items-center justify-center route-state-info px-4 text-main/60">
      <div className="max-w-xl rounded-3xl border border-info/20 bg-surface/60 p-8 text-center shadow-lg backdrop-blur-xl">
        <p className="text-xs text-info">校友地图</p>
        <h1 className="mt-3 text-3xl font-semibold text-info">校友分布暂时无法加载</h1>
        <p className="mt-3 text-sm leading-6 text-main/60">数据连接出现短暂异常，请稍后重试。</p>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <button type="button" onClick={() => reset()} className="rounded-full bg-info px-5 py-2.5 text-sm font-medium text-main/60 transition hover:bg-info">重新加载</button>
