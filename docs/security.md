...
| 接口类型 | 限制 | 理由 |
|----------|------|------|
| 登录/注册/密码重置 | 4KB-16KB | 纯文本字段，很小 |
| 普通管理写入 | 16KB | 新闻、活动、内容块、审核备注等结构化文本 |
| 校友故事提交/后台故事编辑 | 512KB | 长文纯文本投稿，字段层仍限制正文长度 |
...
6. 上传目录
   └── 本地默认 `public/uploads/`，生产推荐 `UPLOAD_DIR=/var/www/alumni-site/uploads`
   └── 目录缺失时图片管道会自动创建，仓库不提交运行时上传文件
```
