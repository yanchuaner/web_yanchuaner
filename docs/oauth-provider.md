...
普通成员跨站登录已由项目负责人在主域确认通过；管理员登录也已在 API 与 AI 的全新浏览器上下文中复验。主站停用/角色变化已通过签名身份事件 webhook 推送到 API（幂等撤销存量 grant 与 Token），自主 AI Web 在上游返回 401/403 时清除本地会话；该链路已在本地隔离环境完成 API 侧验收，三端 staging 联合复验和观察期内的异常登录监控仍须作为后续运行门禁持续验证。
