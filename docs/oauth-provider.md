...
截至 2026-08-01，暑期预览身份提供方已切换到 `https://yanchuaner.cn`。主站管理员的同一 `sub` 已在主域完成两条真实 HTTPS 回调：Open WebUI 复用原管理员记录及其数据，New API 复用原 root、服务 Token、额度与流水；两个子站均未再创建第二个活跃管理员。生产使用独立 Redis 和持久化 RS256 签名密钥，`staging.yanchuaner.cn` 的客户端 Secret 与签名密钥已在受限备份后清空，当前不再签发身份。
...
普通成员跨站登录已由项目负责人在主域确认通过；管理员登录也已在 API 与 AI 的全新浏览器上下文中复验。主站停用/角色变化已通过签名身份事件 webhook 推送到 API（幂等撤销存量 grant 与 Token），自主 ai-web 已上线 `ai.yanchuaner.cn` 并在上游返回 401/403 时清除本地会话；2026-08-12 主站与 API 已发布到生产，生产 webhook 连通性冒烟通过。观察期内的异常登录监控仍须持续验证。
