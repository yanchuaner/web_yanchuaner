...
主站是校友身份的唯一来源。API 平台通过短效 OAuth 授权码获取最小身份声明，不读取主站数据库，也不共享 `yc_access_token` Cookie。
...
只允许邮箱已验证、账号有效且身份为 `ALUMNI + VERIFIED` 的用户；管理员可用于运维验证。授权码在 Redis 中保存 60 秒且只能消费一次，访问令牌保存 5 分钟。生产环境未配置 Redis 或 OAuth 环境变量时端点返回 `temporarily_unavailable`，不会降级为可重放的无状态授权码。
...
部署前还需在真实 Redis 上验证正常登录、未登录跳转、未认证拒绝、错误回调地址、授权码重放和账号停用后的重新授权。
