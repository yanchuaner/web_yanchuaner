...
| `/ai` | 认证校友 | AI 工作台入口，用户身份由主站 OIDC 提供 |
| `/developers` | 认证校友 | API 平台与开发者密钥入口 |
| `/news` | 公开/认证成员 | 列出 `PUBLIC` 新闻；认证校友和管理员可同时查看 `MEMBER` 新闻 |
| `/news/[id]` | 公开/认证成员 | 公开新闻直接阅读；成员新闻要求认证校友或管理员 |
...
> [!NOTE]
> 新闻 API 是“公开可见性 + 成员权限”的混合边界：游客只能读取 `visibility=PUBLIC` 的已发布新闻，认证校友与管理员可读取全部已发布新闻；活动、记忆、故事等内容 API 仍按原有认证策略保护。

---

### 内容 API（公开新闻；成员新闻需认证校友或管理员）

| 路由 | 权限 | 方法 | 说明 |
|------|------|------|------|
| `/api/news` | 公开/认证成员 | GET | 游客获取公开新闻；认证成员获取全部已发布新闻 |
| `/api/news/[id]` | 公开/认证成员 | GET | 成员新闻对未认证请求返回 401 |
...
| 路由 | 权限 | 方法 | Payload 限制 | 说明 |
|------|------|------|-------------|------|
| `/api/admin/news` | 管理员 | GET | — | 新闻列表（支持 `status`、`category`、`visibility` 筛选） |
